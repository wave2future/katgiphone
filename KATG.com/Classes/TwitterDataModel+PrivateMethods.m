...
- (NSMutableArray *)_performParse:(NSURL *)feedURL
{
	...
	for (CXMLElement *element in elements)
	{
		...
		if (tweetText && ![[NSThread currentThread] isCancelled])
		{
			...
			NSArray *urls = [extractor newURLList:tweetText];
			if (urls)
			{
				[tweet setObject:urls forKey:@"urls"];
			}
			NSArray *twts = [extractor newTWTList:tweetText];
			if (twts)
			{
				[tweet setObject:twts forKey:@"twts"];
			}
			[extractor release];
		}
